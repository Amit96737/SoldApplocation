...
// if (document.getElementById('category-products-chart')) {
// 	const category_statics = document.getElementById("category_statics").value.replace(/'/g, '"');
...
// 				name: 'Count',
// 				color: '#1A56DB',
// 				data: JSON.parse(category_statics)
// 			}
// 		],
// 		chart: {
// 			type: 'bar',
...
// 	const chart = new ApexCharts(document.getElementById('category-products-chart'), options);
// 	chart.render();
// }

if (document.getElementById('category-products-chart')) {
	const category_statics = document.getElementById("category_statics").value.replace(/'/g, '"');
	const parsedData = JSON.parse(category_statics);
...
const getSignupsChartOptions = () => {
	...
	return {
		series: [{
			name: 'Users',
			data: JSON.parse(document.getElementById('users_data').value)
		}],
		labels: JSON.parse(document.getElementById('user_labels').value.replace(/'/g, '"')),
		chart: {
			type: 'bar',
			height: '140px',
			foreColor: '#4B5563',
...
if (document.getElementById('top-brand-chart')) {
	const category_statics = document.getElementById("top_brand_stats").value.replace(/'/g, '"')
	.replace(/'/g, '"')
  	.replace(/(\w)"(\w)/g, '$1\'$2');

	console.log("Before parsing:", category_statics);
	const parsedData = JSON.parse(category_statics);
	...
	const chart = new ApexCharts(document.getElementById('top-brand-chart'), options);
	chart.render();
}
